classdef KinematicRegularization < Regularization
    ...
    methods(Access = protected)
        function assemble_matrix_private(obj, solver)
            assert(isa(solver.data_model,"TaylorModel"), "To impose kinematic BCs, a Taylor model is required");
            if (all(solver.data_model.sigma_order > 0) || all(solver.data_model.z_order > 0))
        ...
        function [C, rhsvec] = assemble_kinematic(solver)
            ...
            subtidal_idx = [find(strcmp(model.names, sprintf('%s%s%s', 'u0: M0A'))) , ...
                find(strcmp(model.names, sprintf('%s%s%s', 'd^1u/dsig^1: M0A'))) ,...
                find(strcmp(model.names, sprintf('%s%s%s', 'v0: M0A'))) , ...
                find(strcmp(model.names, sprintf('%s%s%s', 'd^1v/dsig^1: M0A'))) , ...
                find(strcmp(model.names, sprintf('%s%s%s', 'w0: M0A'))) , ...
                find(strcmp(model.names, sprintf('%s%s%s', 'd^1w/dsig^1: M0A')))];
            tidal_idx = cell([numel(model.constituentsU),2]);
            
            for i = 1:numel(model.constituentsU) %Fundamental choice: All constituents are the same!!
                const = model.constituentsU{i};
                tidal_idx{i,1} = [find(strcmp(model.names, sprintf('%s%s%s', 'u0: ', const,'A'))) , ...
                    find(strcmp(model.names, sprintf('%s%s%s', 'd^1u/dsig^1: ', const,'A'))) ,...
                    find(strcmp(model.names, sprintf('%s%s%s', 'v0: ', const,'A'))) , ...
                    find(strcmp(model.names, sprintf('%s%s%s', 'd^1v/dsig^1: ', const,'A'))) , ...
                    find(strcmp(model.names, sprintf('%s%s%s', 'w0: ', const,'A'))) , ...
                    find(strcmp(model.names, sprintf('%s%s%s', 'd^1w/dsig^1: ', const,'A')))];

                tidal_idx{i,2} = [find(strcmp(model.names, sprintf('%s%s%s', 'u0: ', const,'B'))) , ...
                    find(strcmp(model.names, sprintf('%s%s%s', 'd^1u/dsig^1: ', const,'B'))) ,...
                    find(strcmp(model.names, sprintf('%s%s%s', 'v0: ', const,'B'))) , ...
                    find(strcmp(model.names, sprintf('%s%s%s', 'd^1v/dsig^1: ', const,'B'))) , ...
                    find(strcmp(model.names, sprintf('%s%s%s', 'w0: ', const,'B'))) , ...
            ...
            el_mat_size  = [1+2*numel(wl.constituents), sum(model.npars)];
