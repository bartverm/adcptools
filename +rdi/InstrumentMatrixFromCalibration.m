...
%   see also: InstrumentMatrixProvider
    methods(Access=protected)
        ...
        function i2b=get_i2b_matrix(obj,adcp,varargin)
            validateattributes(adcp,{'ADCP'},{'scalar'});
            [b2i, ~, idx]=unique(squeeze(reshape(...
                obj.get_b2i_matrix(adcp),1,adcp.nensembles,[])),'rows'); % get unique matrices
        ...
        function b2i=get_b2i_matrix(~,adcp,varargin)
            validateattributes(adcp,{'ADCP'},{'scalar'});
            b2i=permute(adcp.int16_to_double(...
                adcp.raw.transformation_matrix)/1e4,[4, 3, 2, 1]);
        end
    end
end