...
R=PD0.Reader(cat(1,tmp_dat{:}));                                                 % concatenate raw data

% search for ensembles

% file_id=nan(size(R.ensemble.pos));
% cum_size=[0; cumsum(filesize)];
% for cnt_files=1:nfiles
%     file_id(R.ensemble.pos>=cum_size(cnt_files) & R.ensemble.pos < cum_size(cnt_files+1))=cnt_files;
% end
...
dataout=struct();
obj.fixed_leader=R.read_block(obj.fixed_leader.traits);
obj.variable_leader=R.read_block(obj.fixed_leader.traits);
for cnt_block=1:numel(obj.dynamic_data_type_traits)
    current_data_block_traits=obj.dynamic_data_type_traits(cnt_block);
    tmp_dat=R.read_block(current_data_block_traits);
    if ~isempty(tmp_dat)
        obj.addprop(current_data_block_traits.name);
        obj.(current_data_block_traits.name)=tmp_dat;
    end
end
