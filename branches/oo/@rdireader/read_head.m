...
DataHeader=rdi_headers(ones(NDataTypes,2,'uint16')*65535);                     %Initialize header vector
for cntDataType=1:NDataTypes
    fseek(obj.fid,headpos+DataOffset(cntDataType),-1);                         %Move to data header in ensemble
    DataHeader(cntDataType,:)=rdi_headers(fread(obj.fid,1,'*uint16'));                     %read header of data type (And next byte for navigation data)
end